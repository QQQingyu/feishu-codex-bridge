...
describe('reserved external message classifier', () => {
  ...
  it('reserves common external transcript markers', () => {
    expect(classifyReservedExternalMessage('[study-room:room-1] @Codex here is my note')).toBe(
      'external-marker',
    );
    expect(classifyReservedExternalMessage('[orchestrator:run-1] @Codex note')).toBe(
      'external-marker',
    );
  });
